...
import { getAdapter } from './adapter/app-event.adapter.js';
import { AppEvents, AppEventName } from './app-event-name.js';
import {
...
export function fromAppEvent(
  ...
  return new Observable<AppEventArg>((subscriber) => {
    ...
    // @ts-expect-error the event name is statically typed in electron
